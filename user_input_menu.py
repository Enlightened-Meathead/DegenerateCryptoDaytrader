...
# Syntax will be (program name) --asset --buy_order_type --sell_order_type --
'''
...
def test():
    ...
    # If bot type is exchange, display avaliable exchanges, have them enter an API key and neccesary creds
    ...
    buy_order_type = click.prompt("Select a buy order type", type=click.Choice(["basic", "rsi"]))
    ...
    sell_order_type = click.prompt("Select a sell order type", type=click.Choice(["basic", "ladder"]))
    ...
    minimum_capital = click.prompt("Would you like to set a minimum that you will allow your capital to dip that would trigger the bot to abort any further trades?"
